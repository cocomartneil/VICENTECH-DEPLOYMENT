...
const StaffRecords = () => {
  ...
  const exportRecords = async () => {
    try {
      ...
      setSuccessMessage(`Successfully exported ${activeTab} records to CSV file!`);
      setShowSuccessModal(true);
    } catch (error) {
      console.error('Error exporting records:', error);
      // Show error modal instead of alert
      setSuccessMessage('Failed to export records. Please try again.');
      setShowSuccessModal(true);
    }
  };
  ...
  const handleUpdateRecord = async (recordData) => {
    try {
      const response = await api.put(`/public-parish-records/${recordToEdit.id}`, recordData);
  ...
  const handlePrintRecord = (record) => {
    ...
    const certificateHTML = `
      ...
      <html>
      <head>
        <title>Parish Record Certificate - ${record.name}</title>
        <style>
          @page {
            size: A4;
            margin: 0;
          ...
          .certificate {
            ...
            margin: 0 auto;
            padding: 60px;
            border: 8px double #CD8B3E;
            background: linear-gradient(to bottom, #FFF6E5 0%, #FFFFFF 100%);
            position: relative;
          }
      ...
      <body>
        <div class="certificate">
          <div class="seal">OFFICIAL<br/>PARISH<br/>SEAL</div>
          
          <div class="header">
            <div class="church-name">San Vicente Ferrer Parish</div>
            <div style="font-size: 14px; color: #5C4B38; margin-top: 5px;">
              Mamatid Cabuyao, Laguna
            </div>
          </div>
          
          <div class="certificate-title">Certificate of Record</div>
          ...
          <div class="content">
            ...
            <div class="detail-row">
              ...
              <span class="detail-value">${new Date(record.date).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
              })}</span>
            ...
            <div class="detail-row">
              <span class="detail-label">Officiating Priest:</span>
              <span class="detail-value">${record.priest}</span>
            </div>
          ...
          <div class="footer">
            <div class="signature-block">
              <div class="signature-line">${record.priest}</div>
              <div class="signature-label">Officiating Priest</div>
            </div>
          ...
          <div style="text-align: center; margin-top: 40px; font-size: 12px; color: #5C4B38;">
          ...
          >
            <div style={{ textAlign: 'center' }}>
              ...
              <button
                ...
                onMouseEnter={(e) => e.target.style.backgroundColor = '#B67A35'}
